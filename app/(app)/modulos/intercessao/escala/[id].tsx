...
import { useIntercession, IntercessionAssignment } from '@/hooks/useIntercession'
...
export default function TorreDetalheScreen() {
  const { id } = useLocalSearchParams<{ id: string }>()
  const router = useRouter()
  const insets = useSafeAreaInsets()
  const { upcoming, past, loading, markViewed, respond } = useIntercession()

  const [responding, setResponding] = useState(false)
  const [declineModalVisible, setDeclineModalVisible] = useState(false)
  const [declineReason, setDeclineReason] = useState('')

  const assignment: IntercessionAssignment | undefined =
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      Alert.alert('Presença confirmada!', 'Sua participação na torre foi registrada.', [
    ...
    <>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {ev.notes && (
            <DetailRow icon="document-text-outline" label="Observações" value={ev.notes} />
          )}
          {assignment.notes && (
            <DetailRow icon="chatbubble-outline" label="Notas da escala" value={assignment.notes} />
          )}
        </View>
        ...
        {canRespond && (
          <View style={styles.actions}>
            <Text style={styles.actionsLabel}>Você participará desta torre?</Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  backBtn: { marginTop: spacing.md, paddingHorizontal: spacing.xl, paddingVertical: spacing.sm, backgroundColor: '#EDE9FE', borderRadius: radius.md },
  ...
  typePill: { alignSelf: 'flex-start', paddingHorizontal: spacing.sm, paddingVertical: 3, borderRadius: radius.sm, backgroundColor: 'rgba(255,255,255,0.2)' },
